...
public class Analizy {
     ...
     * Диапазон времени не работы сервера считается от статуса 400-500 до статуса 200-300.
     * indicator - признак начала и окончания считывания периода работы сервера (true - начинаем читать, false - заканчиваем)
    ...
    public void unavailable(String source) {
        try (BufferedReader in = new BufferedReader(new FileReader(source))) {
            in.lines()
                    .forEach(line -> {
                        if ((line.startsWith("400") || line.startsWith("500")) && indicator) {
                            indicator = false;
                            str.append(line.substring(4)).append("; ");
                        }
                        if ((line.startsWith("200") || line.startsWith("300")) && !indicator) {
                            indicator = true;
